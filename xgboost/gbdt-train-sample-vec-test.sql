create table if not exists zhouyf.gbdt_feature_test_vec_weight_sample(pvid string,item_id bigint, label string, price string, sellcnt string, evlcnt string, collectcnt string, addCartCnt string, ctr string,uid_ctr string, refund_rate string, cate_ctr string, cate_weight string, titleSim string, cateSim string, categoryId string, query_id string) partitioned by (ds string)
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' stored as parquet;

insert overwrite table zhouyf.gbdt_feature_test_vec_weight_sample partition(ds='${bizdate}')
select pvid, item_id, label,
concat("1:",cast(ln(nvl(price,0)+1) as string)) as price,
concat("2:",cast(nvl(sellcnt,0) as string)) as sellcnt,
concat("3:",cast(nvl(evlcnt,0) as string)) as evlcnt,
concat("4:",cast(nvl(collectcnt,0) as string)) as collectcnt,
concat("5:",cast(nvl(addCartCnt,0) as string)) as addCartCnt,
