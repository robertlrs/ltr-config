create table if not exists zhouyf.gbdt_feature_train_interval_sample(pvid string,label tinyint,hitattrs string, hittitle string, price string, difftm string, sellcnt string, evlcnt string, collectcnt string, addCartCnt string, ctr string,uid_ctr string, refund_rate string, cate_ctr string, cate_weight string, categoryId string, query_id string) partitioned by (ds string) 
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' stored as parquet;

insert overwrite table zhouyf.gbdt_feature_train_interval_sample partition(ds='${bizdate}')
select pvid, label,
...
concat("11:",cast(nvl(refund_interval_rate,0) as string)) as refund_rate,
