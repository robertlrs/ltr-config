...
create table if not exists zhouyf.gbdt_feature_train_craftsctr_hot_android(pvid string, label tinyint,hitattrs string, hitpreCategoryList string, price string, difftm string, sellcnt string, contenttype string, evlcnt string, collectcnt string, add_cart_cnt string, hittitle string, query_id string, ctr string, uid_ctr string, category_ctr string, categoryId string) partitioned by (ds string)
ROW FORMAT DELIMITED
FIELDS TERMINATED BY '\t'
stored as parquet;

insert into zhouyf.gbdt_feature_train_craftsctr_hot_android partition(ds='${var:statsday}')
select pvid, label,
...
idsToFeatures(cast(isnull(categoryId,0) as string), 14, 868) as categoryId
from
(
...
(select * from zhouyf.item_pv_click_seg_word_train_compliment_android where ds = '${var:statsday}') m left join
(select * from zhouyf.search_item_ctr where ds = '${var:statsday}') n on m.item_id = n.item_id
left join 
(select * from zhouyf.search_uid_ctr where ds = '${var:statsday}')  p on m.uid = p.uid
