...
create function if not exists idsToFeatures(string,int,int) returns string location 'hdfs://hadoop-manager:8020/user/hdfs/wanbin/udf-1.0-SNAPSHOT.jar' symbol='com.dongjia.search.IdsToFeatures';


create table if not exists zhouyf.gbdt_feature_train_craftsctr_hot(pvid string,label tinyint,hitattrs string, hitpreCategoryList string, price string, difftm string, sellcnt string, contenttype string, evlcnt string, collectcnt string, add_cart_cnt string, hittitle string, query_id string, ctr string,uid_ctr string, category_ctr string, categoryId string) partitioned by (ds string)
ROW FORMAT DELIMITED
FIELDS TERMINATED BY '\t'
stored as parquet;

insert into zhouyf.gbdt_feature_train_craftsctr_hot partition(ds='${var:statsday}')
select pvid, label,
...
(
select m.*, n.ctr, p.ctr as uid_ctr, q.ctr as category_ctr from 
(select * from zhouyf.item_pv_click_seg_word_train_compliment where ds = '${var:statsday}') m left join
(select * from zhouyf.search_item_ctr where ds = '${var:statsday}')  n on m.item_id = n.item_id
left join (select * from zhouyf.search_uid_ctr where ds = '${var:statsday}') p on m.uid = p.uid
left join (select * from zhouyf.search_cate_ctr where ds = '${var:statsday}') q on m.categoryid = q.categoryid) a, zhouyf.craftsman b where a.uid = b.uid) temp
